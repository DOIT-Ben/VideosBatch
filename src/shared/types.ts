...
export interface Asset {
  ...
  voiceId?: string;
  voiceProvider?: "volc-tts" | "custom";
  voicePreviewText?: string;
  voicePreviewAudioUrl?: string;
  voicePreviewStatus?: "idle" | "generating" | "ready" | "error";
  voicePreviewError?: string;
  voiceGeneratedAt?: string;
  /** Music node metadata: reusable BGM/song audio generated by the Volcengine Music API. */
  ...
  musicStatus?: "idle" | "generating" | "ready" | "error";
  musicError?: string;
  musicProgress?: string;
  musicGeneratedAt?: string;
  tosObjectKey?: string;
...
export interface Shot {
  ...
  videoUrl?: string;
  /** Preferred browser playback URL, normally a TOS/CDN or signed remote URL. */
  playbackVideoUrl?: string;
  /** Preferred download URL. Falls back to playbackVideoUrl when unset. */
  downloadVideoUrl?: string;
  videoTosObjectKey?: string;
  videoTosPublishedAt?: string;
  renders?: ShotRender[];
...
export interface StitchJob {
  ...
  /** Ready video nodes that are not included in this custom playlist. Snapshot-only metadata. */
  unlistedShotIds?: string[];
  status?: StitchStatus;
  startedAt?: string;
  updatedAt?: string;
  error?: string;
...
export interface Session {
  ...
  finalVideoGeneratedAt?: string;
  finalVideoSignature?: string;
  /** Current signature computed from the legacy stitch playlist and latest selected video inputs. Snapshot-only metadata. */
  currentStitchSignature?: string;
  /** True when the legacy final video no longer matches the current playlist/video inputs. Snapshot-only metadata. */
  finalVideoStale?: boolean;
  ...
  /** Ready video nodes that are outside the custom legacy stitch playlist. Snapshot-only metadata. */
  stitchUnlistedShotIds?: string[];
  /**
  ...
  musicLocalAudioUrl?: string;
  /**
   * Optional post-stitch stem separation. Runs on the selected stitched final video and writes
   * separate human-voice and background audio files that remain visible from the audio-track
   * Inspector. The default local implementation is an ffmpeg center-vocal fallback; deployments
   * can override it with `AUDIO_SEPARATION_COMMAND`.
   */
  audioSeparationStatus?: AudioSeparationStatus;
  audioSeparationStartedAt?: string;
  audioSeparationUpdatedAt?: string;
  audioSeparationError?: string;
  audioSeparationProgress?: string;
  audioSeparationVocalsUrl?: string;
  audioSeparationBackgroundUrl?: string;
  audioSeparationSignature?: string;
  audioSeparationRunningSignature?: string;
  audioSeparationStitchJobId?: string;
  audioSeparationBuiltForFinalVideoSignature?: string;
  audioSeparationMethod?: string;
  /**
