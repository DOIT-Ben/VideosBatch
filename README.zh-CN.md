...
在线上部署里，把返回里的 `handoffUrl` 发给用户，不要只发 raw `webUrl`。CLI 和浏览器用不同 cookie 身份隔离，`handoffUrl` 才能让普通浏览器接管 AI 创建的 workflow。在 `localhost`、`127.0.0.1` 或 `::1` 本地运行时，直接返回显式 `webUrl`；CLI 创建的本地 session 会自动出现在浏览器 session 列表里，跨 cookie 身份可见，不需要加密 handoff token。
...
- 用户只给剧本灵感时，agent 应先调研相关角色、情节机制和历史背景，再写初稿；初稿后做多轮剧本 review 并持续修改，直到 review 满意。除非用户明确要求交互讨论，否则 agent 默认自己闷头完成并交付可审阅 canvas。
- Agent 应尽量使用完整 15 秒 Seedance 分镜，在角色、场景、光线、情绪连续时把多个相关 beats 放进同一个视频，而不是每个 beat 都拆成更短视频。
- Agent 生成前应先规划镜头语言：镜头运动要有叙事动机，人物调度和屏幕方向要清楚，相邻镜头用动作、视线、反应、道具特写、切出镜头、声音、previous-tail 或尾帧接力来衔接。
- prompt 里的关键信息要让观众听见或看懂：用人物对白或旁白/叙述传达，并用可见动作、道具或反应兜底，不要只藏在 prompt 设定或字幕里。
